...
const roleIconMap = {
  ...
  ADMIN: <Shield className="h-4 w-4 ml-2 text-rose-500" />,
};
  ...
  return (
    <Dialog open={isModalOpen} onOpenChange={onClose}>
      <DialogContent className="bg-white dark:bg-[#313338] p-0 overflow-hidden">
        ...
        <ScrollArea className="m-8 max-h-[420px] pr-6">
          {server?.members?.map((member) => (
            <section key={member.id} className="flex items-center gap-x-2 mb-6">
              ...
              <div className="flex flex-col gap-y-1">
                ...
                <p className="text-xs text-zinc-700 dark:text-[#B1B6BD]">
              ...
              {server.profileId !== member.profile.id &&
                loadingId !== member.id && (
                  <div className="ml-auto">
                    <DropdownMenu>
                      <DropdownMenuTrigger>
                        <MoreVertical className="h-4 w-4 text-[#B1B6BD]" />
              ...
              {loadingId === member.id && (
                <Loader2 className="animate-spin text-[#B1B6BD] ml-auto w-4 h-4" />
              )}
            </section>
          ))}
        </ScrollArea>
